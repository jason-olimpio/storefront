...
const Input = ({
  ...
  useEffect(() => setIsActive(!!value.trim()), [value])

  const handleAutofill = (event: React.AnimationEvent<HTMLInputElement>) => {
    if (event.animationName === 'onAutoFillStart') setIsActive(true)
  }
  ...
  const handleBlur = () => setIsActive(!!value.trim())
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <input
        ...
        onFocus={handleFocus}
        onBlur={handleBlur}
        onAnimationStart={handleAutofill} // ✅ Autofill detection
        required={required}
        minLength={minLength}
        maxLength={maxLength}
